...
// prompts never have `handoff` (no such column - supabase/README.md) or a
// slug-shaped `depends_on` (Supabase's is a UUID FK, not a slug); both are
// markdown/v3-only fields for now.
...
export function validatePrompts(prompts) {
  ...
  for (const p of prompts) {
    ...
    // depends_on is slug-shaped only for markdown prompts - Supabase's
    // column is a UUID FK (see the loadPrompts() comment above), and
    // nothing sets it yet (not collected by the New Prompt modal), so this
    // check is currently a no-op for Supabase-sourced prompts in practice.
    if (p.depends_on && !slugSet.has(p.depends_on)) {
      errors.push(`${label}: "depends_on: ${p.depends_on}" does not match any known prompt slug`);
    }
    if (slugs.has(p.slug)) {
