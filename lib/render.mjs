...
export function renderPromptCard(p, ctx = {}) {
  ...
  const catChips = p.categories.map(c => renderChip(categoryLabel(c))).join('');
  ...
  return `
<a href="/prompt/${p.slug}/" class="prompt-card surface" data-categories="${esc(cats)}" data-model="${esc(models)}" data-complexity="${esc(p.complexity || '')}" data-chain="${isChain}" data-title="${esc(p.title)}" data-updated="${esc(p.updated || p.added || '')}">
  <div class="card-top">
  ...
  <div class="card-chips">${catChips}${tagChips}${seqChip}</div>
...
<div class="pd-seq-row">
  <span class="pd-seq-current">${esc(info.label)} · Step ${info.step} of ${info.total}</span>
...
// Each row carries the same data-* attributes cards use (model/complexity/
// chain/title/updated) so the existing filter-rail script (filters.js) works
// unchanged, plus data-categories for the new pill filter. Full prompt data
// (body, notes, sequence info) is embedded as a JSON script tag for the
// quick-view modal (quickview.js) to read - a static site has no API to
// fetch it from on click.
export function renderPromptTableRows(prompts, ctx = {}) {
  return prompts.map(p => {
    const total = ctx.sequenceTotals?.[p.sequence];
    const seqTag = p.sequence && total ? `<span class="seq-tag">Step ${p.sequence_step} of ${total}</span>` : '';
    const models = (p.models || []).join(',');
    const cats = p.categories.join(',');
    return `
<tr data-slug="${esc(p.slug)}" data-categories="${esc(cats)}" data-model="${esc(models)}" data-complexity="${esc(p.complexity || '')}" data-chain="${p.sequence ? 'true' : 'false'}" data-title="${esc(p.title)}" data-updated="${esc(p.updated || p.added || '')}" tabindex="0" role="button" aria-label="Open ${esc(p.title)}">
      ...
      <div class="pd-bento">
        ...
        <div class="pd-box surface pd-notes-box">
          ...
          <div id="qv-seq-block" hidden>
            <div class="pd-notes-divider"></div>
            <div class="pd-seq-row">
              <span class="pd-seq-current" id="qv-seq-current"></span>
...
initFilters('${filterId}', 'result-grid');
...
<div class="container">
  ...
  <div class="layout-with-rail">
    ${renderFilterRail(filterId)}
    ${renderPromptTable(prompts, { sequenceTotals: data.sequenceTotals, gridId: 'result-grid', data })}
  </div>
</div>`;
  const script = `<script type="module">
import { initFilters } from '/scripts/filters.js';
...
<div class="container">
  ...
  <div class="layout-with-rail">
    ...
    <div>
      ${renderCategoryFilterBar(data.categories, { id: 'search-category-filter' })}
