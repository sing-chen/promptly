...
<aside class="sidebar" id="sidebar" aria-label="Site navigation">
  ...
  <div class="sidebar-section">
    <div class="sidebar-section-label-row">
      <!-- Two states in the static markup, toggled by setNavAccountState -
           the same pattern as the account footer, and for a related reason
           (§9ak).
           Signed IN, the gear beside this goes to /categories/, so a linked
           label puts two controls with one destination side by side. The
           label loses its link and becomes what it looks like: a section
           heading.
           Signed OUT there is no gear - it is hidden for anonymous visitors -
           and /categories/ has a real signed-out state (categories.js's
           signedOutView: what categories are, and why logging in gets you
           your own). Unlinking the label in both states would leave that page
           reachable only by typing the URL. So the link survives exactly
           where it is the ONLY route, and disappears exactly where it is a
           duplicate.
           Collections keeps its link unconditionally: its "+" opens a modal
           rather than navigating, so the label is the only way to that page
           in either state. -->
      <span class="sidebar-section-label-group">
        ...
        <span class="sidebar-section-label" id="nav-categories-label-static" hidden>Categories</span>
      </span>
