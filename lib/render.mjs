...
<div class="sidebar-callout">
  ...
  // Categories render in full at build time (a fixed, small vocabulary
  // today - lib/schema.mjs CATEGORIES - so this cap rarely if ever fires in
  // practice), but the sidebar must never grow its own scrollbar just
  ...
  const catLink = c => `
<a href="/browse/${c.slug}/"${activeKey === 'category' && activeSlug === c.slug ? ' aria-current="page"' : ''}><span class="nav-icon" aria-hidden="true">${ICON.category}</span><span class="nav-label">${esc(categoryLabel(c.slug))}</span><span class="count">${c.count}</span></a>`;
    ...
    <div class="np-page">
      <form id="np-form" class="account-form np-form">
        ...
        <label class="np-admin-check" id="np-admin-row" hidden>
          ...
          <span>
            Publish to everyone
            <span class="np-admin-hint" id="np-admin-hint">Adds this to the catalog as a draft — copied to every user's library once you publish it from the Admin page. Leave unticked to keep it personal to you.</span>
          </span>
        </label>
...
function personalizedActions(p, ctx) {
  ...
  const btn = (attr, label, title, icon) =>
    `<button type="button" class="icon-btn" data-${attr}="${esc(p.id)}" aria-label="${label} ${esc(p.title)}" title="${title}">${icon}</button>`;
  return `
...
<div class="prompt-table-block" data-grid-id="${esc(gridId)}">
  ...
  <div class="table-wrap">
    <table class="browse-table comfortable">
      <thead>
        <tr><th class="col-select"><input type="checkbox" class="bulk-select-all" aria-label="Select all prompts"></th><th style="width:180px;">Category</th><th>Title</th><th class="col-updated" style="width:100px;">Updated</th><th class="col-actions"></th></tr>
      </thead>
      ...
      <div class="pd-bento">
        <div class="pd-box surface pd-prompt-box">
          <div class="pd-box-label">
            <span>Prompt</span>
            <div class="pd-actions">
              <button type="button" class="pd-action-btn fav" id="qv-fav" aria-label="Favorite" aria-pressed="false" title="Add to Favorites">
                ${ICON.star}<span class="fav-label-off">Favorite</span><span class="fav-label-on">Favorited</span>
              </button>
              <button type="button" class="pd-action-btn square" id="qv-copy" aria-label="Copy prompt" title="Copy Prompt">${ICON.copy}</button>
              <button type="button" class="pd-action-btn square" id="qv-edit" aria-label="Edit" title="Edit" hidden>${ICON.edit}</button>
              <button type="button" class="pd-action-btn square" id="qv-duplicate" aria-label="Duplicate" title="Duplicate — make an independent copy you can vary" hidden>${ICON.duplicate}</button>
              <button type="button" class="pd-action-btn square" id="qv-archive" aria-label="Archive" title="Archive — hides this prompt from your library, reversible" hidden>${ICON.archive}</button>
              <button type="button" class="pd-action-btn square" id="qv-delete" aria-label="Delete" title="Delete — permanently removes this prompt" hidden>${ICON.trash}</button>
            </div>
          </div>
