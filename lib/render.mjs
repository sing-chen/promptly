...
// Exported for public/scripts/auth.js's show-password toggle, which builds its
// markup client-side and so cannot go through a render function here.
export const ICON = {
  ...
  // Was `category:` and was referenced by nothing - a folder icon defined for
  // a sidebar list that uses colour dots instead. Renamed rather than deleted
  // in §9ao, because collectionsNav.js now wants exactly this shape and the
  // sidebar's Collections copy calls them folders in as many words. Sized 16
  // to sit with the other nav icons (stack/star/flow) rather than the 15px
  // row actions.
  collection: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" width="16" height="16"><path d="M3 7a2 2 0 012-2h4l2 2h8a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2V7z"/></svg>',
  ...
  // Sidebar "manage categories" (§9aj). Deliberately NOT `plus` - that icon
  // sits on the Collections button directly above, where it opens a create
  // modal and tells the truth. This one has only ever navigated to
  // /categories/, so a plus promised a creation step that does not exist.
  // 15px to match the other sidebar action icons rather than the 14px `plus`
  // it replaces; a gear reads smaller than a plus at the same box size,
  // having no full-width strokes.
  gear: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" width="15" height="15"><circle cx="12" cy="12" r="3.2"/><path d="M19.4 15a1.65 1.65 0 00.33 1.82l.06.06a2 2 0 11-2.83 2.83l-.06-.06a1.65 1.65 0 00-1.82-.33 1.65 1.65 0 00-1 1.51V21a2 2 0 11-4 0v-.09A1.65 1.65 0 008 19.4a1.65 1.65 0 00-1.82.33l-.06.06a2 2 0 11-2.83-2.83l.06-.06A1.65 1.65 0 004.6 15a1.65 1.65 0 00-1.51-1H3a2 2 0 110-4h.09A1.65 1.65 0 004.6 9a1.65 1.65 0 00-.33-1.82l-.06-.06a2 2 0 112.83-2.83l.06.06A1.65 1.65 0 009 4.6a1.65 1.65 0 001-1.51V3a2 2 0 114 0v.09a1.65 1.65 0 001 1.51 1.65 1.65 0 001.82-.33l.06-.06a2 2 0 112.83 2.83l-.06.06A1.65 1.65 0 0019.4 9a1.65 1.65 0 001.51 1H21a2 2 0 110 4h-.09a1.65 1.65 0 00-1.51 1z"/></svg>',
  // The show/hide-password pair (§9ai). Used by public/scripts/auth.js, which
  // is why ICON is exported at all - every other icon here is consumed by a
  // render function in this file. Kept here rather than defined locally in
  // auth.js so there is one icon set, not two.
  //
  // Sized 16px to match the field they sit in rather than the 15px row
  // actions. The convention is the same one the theme toggle uses: the icon
  // names the ACTION, not the current state - the open eye means "show it",
  // and appears while the password is masked.
  eye: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" width="16" height="16"><path d="M2 12s3.6-7 10-7 10 7 10 7-3.6 7-10 7-10-7-10-7z"/><circle cx="12" cy="12" r="3"/></svg>',
  eyeOff: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" width="16" height="16"><path d="M17.94 17.94A10.07 10.07 0 0112 20c-7 0-10-8-10-8a18.45 18.45 0 015.06-5.94M9.9 4.24A9.12 9.12 0 0112 4c7 0 10 8 10 8a18.5 18.5 0 01-2.16 3.19M14.12 14.12a3 3 0 11-4.24-4.24"/><line x1="2" y1="2" x2="22" y2="22"/></svg>',
...
export function renderNav(active, data) {
  ...
  // no build-time collections list at all, so signed out this list is empty.
  //
  // It used to hold a .sidebar-callout repeating what collections are and
  // linking to /why-sign-in/. That text now lives in the section's (i) hint
  // (§9al), which says the same thing in the same place for every visitor
  // instead of only for logged-out ones. collectionsNav.js still captures
  // this as staticHtml and restores it on sign-out - now restoring empty,
  // which is correct: a signed-out visitor has no collections to list.
...
<aside class="sidebar" id="sidebar" aria-label="Site navigation">
  ...
  <div class="sidebar-section">
    <div class="sidebar-section-label-row">
      <!-- A heading, not a link (§9al). Both section labels lost their links
           together, which is the point: the previous arrangement made
           Categories a link signed out and inert signed in, so the same word
           behaved differently depending on who was looking at it.
           Nothing is lost by unlinking, because the gear beside it goes to
           the same page - see the note on that button. -->
      <span class="sidebar-section-label-group">
        <span class="sidebar-section-label">Collections</span>
        ${fieldInfo('collections-info-hint', 'Collections group your prompts into folders.', {
          btnClass: 'sidebar-info-btn', hintClass: 'sidebar-info-hint', btnId: 'collections-info-btn',
          guest: { id: 'collections-info-guest', text: 'Log in to create and manage them.' }
        })}
      </span>
      <!-- A gear, mirroring Categories exactly (§9am). It replaced a "+" that
           opened a create-collection modal.
           The modal was not wrong, but having one section create in place and
           the other navigate meant two identical-looking buttons doing
           different things, which is what §9aj set out to fix and only half
           fixed. /collections/ already renders its own create form at the top
           (collections.js), so creation survives at one extra click - the
           same trade Categories has always made, and the reason its own
           editor was never duplicated into a modal.
           It also closes a hole the "+" left: collectionsNav.js links to
           /collections/ only once you HAVE a collection, so a signed-in
           account with none had no route to that page at all. A gear does not
           care how many you have. -->
      <button type="button" id="manage-collections-btn" class="icon-btn sidebar-section-add" hidden aria-label="Manage collections" data-tip="Manage collections"${activeKey === 'collections' ? ' aria-current="page"' : ''}>${ICON.gear}</button>
    </div>
  ...
  <div class="sidebar-section">
    <div class="sidebar-section-label-row">
      <!-- Also a heading now (§9al), for the reason given on Collections
           above. §9ak's session-dependent link is gone: it solved the
           duplicate-destination problem by creating a worse one, where the
           label was a control for logged-out visitors and inert text for
           everyone else. -->
      <span class="sidebar-section-label-group">
        <span class="sidebar-section-label">Categories</span>
        ${fieldInfo('categories-info-hint', 'View prompts by category. Prompts can belong to multiple categories.', {
          btnClass: 'sidebar-info-btn', hintClass: 'sidebar-info-hint', btnId: 'categories-info-btn',
          // Same split as Collections above: the half that is only true for a
          // visitor without an account is separated out and hidden once there
          // is one.
          guest: { id: 'categories-info-guest', text: 'Log in to create and manage them (reorder, rename, set colours, delete).' }
        })}
      </span>
      <!-- "Manage", not "New" (§9aj). This button has never created anything -
           categoriesNav.js sends it to /categories/ - so a plus icon labelled
           "New category" made three separate promises the control does not
           keep, while looking identical to the Collections plus above it,
           which does open a create modal. Creation still exists, one click
           further on, via that page's own "+ New category" button.
           The id says manage- for the same reason; #new-category-btn read as
           a create control to anyone grepping for one. -->
      <button type="button" id="manage-categories-btn" class="icon-btn sidebar-section-add" hidden aria-label="Manage categories" data-tip="Manage categories"${activeKey === 'categories' ? ' aria-current="page"' : ''}>${ICON.gear}</button>
    </div>
...
// heading (Prompt Detail, Account) omit them and just get the controls,
// rather than showing a duplicate title. Admin used to be in that list and
// moved out in §9ay.
...
// Hint text only, always esc()'d - §9al briefly allowed a link in here and
// §9am took it back out. The links existed to carry routes the section labels
// had given up; a gear on each section carries them instead, which is both
// simpler and better placed. There is no `html` escape hatch on purpose: the
// escaping is what makes this safe to hand arbitrary strings.
//
// aria-expanded reflects the pinned state, flipped by public/scripts/
// fieldInfo.js. It is in the static markup so the control never announces
// itself as a plain button for the moment before that script runs.
// `guest` appends a second sentence that auth.js hides once there is a
// session (§9an) - "Log in to create one" is advice, and advice aimed at
// someone who has already taken it is just noise in a tooltip that is trying
// to be brief. Structured rather than an HTML option for the same reason cta
// was: esc() stays on everything.
function fieldInfo(id, hintText, { btnClass = '', hintClass = '', btnId = '', btnHidden = false, guest = null } = {}) {
  const guestHtml = guest
    ? ` <span class="field-hint-guest"${guest.id ? ` id="${guest.id}"` : ''}>${esc(guest.text)}</span>`
    : '';
  return `<button type="button" class="field-info${btnClass ? ' ' + btnClass : ''}"${btnId ? ` id="${btnId}"` : ''}${btnHidden ? ' hidden' : ''} aria-describedby="${id}" aria-expanded="false">${ICON.info}<span class="sr-only">More info</span></button><span class="field-hint${hintClass ? ' ' + hintClass : ''}" id="${id}" role="tooltip">${esc(hintText)}${guestHtml}</span>`;
}
...
// renderNewCollectionModal() lived here until §9am. It was opened by the
// sidebar's Collections "+", which is now a gear that navigates to
// /collections/ - where the same form already exists. With no trigger left
// the modal was unreachable markup shipped on every page, so it went with
// the button, along with public/scripts/newCollection.js. Recoverable from
// git if the fast path is ever wanted back.
...
<script type="module" src="${av('/scripts/fieldInfo.js')}"></script>
${extraScripts}
</body>
</html>
`;
...
// data-collections, the collection counterpart of data-categories (§9ap).
// Emitted only when the caller supplies the map - the static build has no
// collections at all (they are user-generated, per-account, and never
// curated), so an anonymous page would otherwise carry an empty attribute on
// every row for nothing.
function collectionAttr(p, ctx) {
  const slugs = ctx.collectionSlugsByPromptId?.get(p.id);
  return slugs?.length ? ` data-collections="${esc(slugs.join(','))}"` : '';
}
...
export function renderPromptCard(p, ctx = {}) {
  ...
  return `
<a href="/prompt/${p.slug}/" class="prompt-card surface" data-slug="${esc(p.slug)}" data-categories="${esc(cats)}"${collectionAttr(p, ctx)}>
  <div class="card-top">
...
<div class="pd-seq-row">
  ...
  // Same row as the modal's (§9aq), and empty in the static build for the
  // same reason: collections are per-account, so there is nothing to render
  // until promptCollections.js has a session to read. Hidden until then.
  const collectionsRow = `
  <div class="pd-collections-row" id="pd-collections-row" hidden>
    <span class="pd-collections-label">In collections</span>
    <span class="pd-collections-list" id="pd-collections-list"></span>
  </div>`;
...
<div class="container pd-page">
  ...
  return renderLayout({ title: prompt.title, description: prompt.purpose, bodyHtml: body, data,
    extraScripts: `<script type="module" src="${av('/scripts/promptCollections.js')}"></script>` });
}
...
// /by-category/ and /by-collection/ (§9aq). Home used to be all three screens
// at once: the sidebar sent category links to /?cat= and collection links to
// /?collection=, so one page titled "All Prompts" had to carry both pill rows
// and served as the destination for two things that are not "all prompts".
//
// Splitting them is what lets each carry ONE filter row. The pages are
// otherwise Home minus the "N new prompts" callout, which belongs to the
// screen that means "everything" and nowhere else - on a filtered view it
// announces prompts that may not even be in the filtered set.
//
// Built from one function because the two differ in four strings and which
// group they render. Two near-identical copies is how the next change lands
// in one of them only.
function renderFilteredPromptsPage(data, { route, groupKey, groupLabel, title, subtitle, options }) {
  const gridId = `${route}-grid`;
  const toolbarId = `${route}-filters`;
  const body = `
<div class="container">
  <hr class="home-hero-divider">
  ${renderFilterToolbar([{ key: groupKey, label: groupLabel, options }], { id: toolbarId })}
  ${renderPromptTable(data.prompts, { sequenceTotals: data.sequenceTotals, gridId, emptyText: 'No prompts match.', data })}
</div>`;
  const script = `<script type="module">
import { initTableFilterToolbar } from '${av('/scripts/filters.js')}';
import { initQuickView } from '${av('/scripts/quickview.js')}';
...
registerQuickView('${gridId}', initQuickView('${gridId}'));
initPersonalizedTable({ gridId: '${gridId}', toolbarId: '${toolbarId}', filterGroups: ['${groupKey}'], sequenceTotals: ${JSON.stringify(data.sequenceTotals)} });
</script>`;
  return renderLayout({
    title, description: subtitle, active: route,
    bodyHtml: body, extraScripts: script, data,
    pageTitle: title, pageSubtitle: subtitle
  });
}

export function renderByCategoryPage(data) {
  return renderFilteredPromptsPage(data, {
    route: 'by-category', groupKey: 'category', groupLabel: 'Category Filter(s)',
    title: 'Prompts by Category',
    subtitle: 'Pick a category to see what is filed under it. A prompt can belong to more than one.',
    // Statically the catalog's categories, replaced by the caller's own once
    // personalize.js resolves - the same arrangement Home has.
    options: data.categories.filter(c => c.count > 0)
      .map(c => ({ value: c.slug, label: categoryName(c), count: c.count, color: c.color }))
  });
}

export function renderByCollectionPage(data) {
  return renderFilteredPromptsPage(data, {
    route: 'by-collection', groupKey: 'collection', groupLabel: 'Collection(s)',
    title: 'Prompts by Collection',
    subtitle: 'Pick a collection to see the prompts you have grouped into it.',
    // Always empty at build time, and necessarily so: collections are
    // user-generated per account and no catalog set exists. renderFilterToolbar
    // omits a group with no options, so an anonymous visitor who reaches this
    // URL directly gets no pill row rather than an empty one. Nothing links
    // them here - collectionsNav.js only renders links once signed in.
    options: []
  });
}
...
export function renderPromptTableRows(prompts, ctx = {}) {
  return prompts.map(p => {
    ...
    const cats = categorySlugs(p).join(',');
    return `
<tr data-slug="${esc(p.slug)}" data-categories="${esc(cats)}"${collectionAttr(p, ctx)} tabindex="0" role="button" aria-label="Open ${esc(p.title)}">
    ...
    <div class="container pd-page">
      ...
      <div class="pd-bento">
        ...
        <div class="pd-box surface pd-notes-box">
          ...
          <div class="pd-notes-divider"></div>
          <!-- Which of the caller's collections hold this prompt (§9aq).
               Above the dates and below the hairline, and hidden unless there
               is something to say - a "Collections: none" row would be noise
               on the many prompts that are in none, and meaningless to the
               anonymous visitors who have no collections at all.
               Filled by quickview.js from the personalization cache; there is
               nothing to server-render, since collections are per-account. -->
          <div class="pd-collections-row" id="qv-collections-row" hidden>
            <span class="pd-collections-label">In collections</span>
            <span class="pd-collections-list" id="qv-collections-list"></span>
          </div>
          <div class="pd-meta-row">
...
// to change with it.
//
// One such change is already scheduled and worth naming so it is not missed:
// configuring custom SMTP (OPEN_ITEMS.md A1) puts a *third* processor between
// this service and the user - the email provider, which necessarily sees the
// address every confirmation and reset goes to. Section 7's list of who else is
// involved says "Supabase" for service email today, and that stops being true
// the moment the SMTP settings are filled in. The provider has not been chosen
// yet, which is the only reason the list is not already longer.
//
...
<div class="container prose-page why-sign-in-page">
  ...
  <section id="why-signup-asks">
    ...
    <!-- Added in §9ar. The address has to work now rather than merely be
         well-formed, which is a real change in what is being asked for and
         belongs on the page that answers "what will this cost me". The second
         sentence is the one that earns its place: it is the only reassurance
         that "no marketing list" survives contact with an inbox. -->
    <p>The email address has to be one you can read: signing up sends you a link
    that activates the account, and you cannot log in until you have followed
    it. That mail, and a password reset if you ever ask for one, are the only
    emails Promptly will ever send you.</p>
  </section>
...
// Static shell only, like /account/ - public/scripts/resetPassword.js fills
// #reset-root once it knows whether the link that brought the visitor here
// produced a session.
//
// This page exists because a password-reset email has to land *somewhere* in
// this site, and that somewhere has to be listed under Authentication → URL
// Configuration → Redirect URLs in the Supabase dashboard. It is not linked
// from the nav and never will be: every route to it is either an emailed link
// or a signed-in visitor deliberately typing it, which is why it carries its
// own explanation rather than assuming context.
//
// No pageTitle/pageSubtitle - the page has three quite different states (set a
// password, link is dead, password changed) and each renders its own h1. A
// fixed header above them would be describing whichever one it was written
// for.
export function renderResetPasswordPage(data) {
  const body = `
<div class="container account-page">
  <div id="reset-root">
    <p style="color:var(--ink-faint);">Checking your link…</p>
  </div>
</div>`;
  const script = `<script type="module" src="${av('/scripts/resetPassword.js')}"></script>`;
  return renderLayout({
    title: 'Set a new password',
    description: 'Choose a new password for your Promptly account.',
    active: 'account',
    bodyHtml: body,
    extraScripts: script,
    data
...
<div class="container account-page admin-page">
  ...
  return renderLayout({
    ...
    extraScripts: script,
    data,
    // §9ay: was a client-rendered <h1> in the body, which is why the header
    // comment above still lists Admin among the pages that "omit them and just
    // get the controls". It now follows /categories/ and /collections/ - title
    // and one-line summary in the header, the detailed explanation in the body
    // where the h1 used to sit. The summary and §9ax's opening line have to
    // stay distinct: two restatements of the same sentence stacked on top of
    // each other is what the old arrangement looked like.
    pageTitle: 'Admin',
    pageSubtitle: 'Publish prompts from your library to everyone, or hold them as drafts.'
  });
}
...
// Deliberately the same shell as renderCategoriesPage above (§9an), down to
// the pageTitle/pageSubtitle pair this used to omit entirely - the page
// opened straight onto a form with no heading explaining what it was for,
// while /categories/ had both. The two pages manage the same *kind* of thing
// and now say so in the same way.
//
// renderQuickViewModal() is new here: each collection lists its prompts as
// [data-slug] links, and collections.js hands the whole list to
// initQuickView() so clicking one opens the prompt modal in place. Without
// this markup that call finds no #qv-backdrop and quietly does nothing.
...
${renderQuickViewModal()}`;
  ...
  return renderLayout({
    ...
    pageSubtitle: 'Group prompts for a project or workflow, and pull them all up at once.'
  });
}
