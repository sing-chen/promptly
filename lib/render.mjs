...
<aside class="sidebar" id="sidebar" aria-label="Site navigation">
  <div class="sidebar-brand-row">
    ...
    <button type="button" id="sidebar-collapse-btn" class="icon-btn" aria-label="Collapse sidebar" title="Collapse sidebar">${ICON.panelCollapse}</button>
  </div>
...
<header class="main-header">
  <div class="main-header-left">
    <button type="button" id="sidebar-expand-btn" class="icon-btn" aria-label="Expand sidebar" title="Expand sidebar" hidden>${ICON.panelExpand}</button>
    ${pageTitle ? `
...
// `ctx.personalized` marks a mixed-ownership list (the merged catalog a
// signed-in user sees - default prompts they haven't forked, plus their own)
// - every row/card gets an Edit button. Archive only makes sense for a
// default (is_curated) - it's the "archive without editing" path
// (supabase/README.md's "Admin curation...", db.js's archiveDefault()); a
// user's own prompt has no separate archived state, just Delete, which only
// shows for a row the caller actually owns (editing a default forks it
// rather than editing it in place). `ctx.currentUserId` is required
// alongside `ctx.personalized` to tell owned rows apart from defaults.
function personalizedActions(p, ctx) {
  if (!ctx.personalized) return '';
  const isOwn = p.user_id === ctx.currentUserId && !p.is_curated;
  const archive = p.is_curated ? `<button type="button" class="icon-btn" data-archive-id="${esc(p.id)}" aria-label="Archive ${esc(p.title)}" title="Archive — hides this prompt from your view and filters">${ICON.archive}</button>` : '';
  const del = isOwn ? `<button type="button" class="icon-btn" data-delete-id="${esc(p.id)}" aria-label="Delete ${esc(p.title)}" title="Delete — permanently removes this prompt">${ICON.trash}</button>` : '';
  return `
      <button type="button" class="icon-btn" data-edit-id="${esc(p.id)}" aria-label="Edit ${esc(p.title)}" title="Edit">${ICON.edit}</button>${archive}${del}`;
}
...
<a href="/prompt/${p.slug}/" class="prompt-card surface" data-slug="${esc(p.slug)}" data-categories="${esc(cats)}">
  <div class="card-top">
    ...
    <span class="card-actions">
      <button type="button" class="icon-btn fav" data-fav-slug="${esc(p.slug)}" aria-label="Favorite ${esc(p.title)}" aria-pressed="false" title="Add to Favorites">${ICON.star}</button>
      <button type="button" class="icon-btn" data-copy-target="prompt-copy-${esc(p.slug)}" aria-label="Copy ${esc(p.title)}" title="Copy Prompt">${ICON.copy}</button>${personalizedActions(p, ctx)}
    </span>
  </div>
...
// perpetually new. Exported so personalize.js can recompute this after a
// signed-in merge - build-time `data.prompts` is already only published
// defaults, so `p.is_curated` is undefined there and this check is a no-op;
// a personalized `merged` list carries real is_curated booleans, and
// excluding `false` is what keeps a user's own prompts/forks from counting
// as "new defaults", and (since an archived default drops out of `merged`
// entirely) what stops an already-forked-and-archived default from still
// being announced to the very user who archived it by editing it.
...
${newCalloutScript}
  ...
  const stats = `${data.prompts.length} prompts across ${data.categories.length} categories, organized into ${data.sequences.length} sequence${data.sequences.length === 1 ? '' : 's'}.`;
  return renderLayout({ title: 'Promptly', description: 'Personal AI prompt catalog.', active: 'home', bodyHtml: body, extraScripts: script, data, pageTitle: 'All Prompts', pageSubtitle: stats });
}
...
export function toQuickViewItem(p, ctx = {}) {
  return {
    ...
    // real /prompt/[slug]/ page - only a personalized (signed-in, merged
    // catalog) item passes these explicitly, since a personal/forked prompt
    // (is_curated=false) has no static page for quickview.js's "Open full
    // page" link to point at.
...
<div class="prompt-table-block" data-grid-id="${esc(gridId)}">
  <div class="table-wrap">
    <table class="browse-table comfortable">
      <thead>
        <tr><th style="width:180px;">Category</th><th>Title</th><th class="col-updated" style="width:100px;">Updated</th><th style="width:150px;"></th></tr>
      </thead>
...
<div class="backdrop" id="qv-backdrop">
  <div class="qv-modal" id="qv-modal" role="dialog" aria-modal="true" aria-labelledby="qv-title">
    <div class="qv-chrome">
      ...
      <div class="qv-chrome-right">
        <a href="#" class="qv-open-full" id="qv-view-original" hidden>View original ↗</a>
        ...
        <div class="pd-box surface pd-prompt-box">
          <div class="pd-box-label">
            ...
            <div class="pd-actions">
              ...
              <button type="button" class="pd-action-btn square" id="qv-edit" aria-label="Edit" title="Edit" hidden>${ICON.edit}</button>
              <button type="button" class="pd-action-btn square" id="qv-delete" aria-label="Delete" title="Delete" hidden>${ICON.trash}</button>
            </div>
          </div>
...
initPersonalizedTable({ gridId: 'result-grid', filterFn: p => p.categories.includes(${JSON.stringify(category)}), sequenceTotals: ${JSON.stringify(data.sequenceTotals)} });
</script>`;
  return renderLayout({ title: categoryLabel(category), description: `Prompts in ${categoryLabel(category)}.`, active: `category:${category}`, bodyHtml: body, extraScripts: script, data, pageTitle: categoryLabel(category), pageSubtitle: description });
}
...
// prompts" and "archived defaults" - both of those are gone now that the
// merged catalog on Home/Category/Search *is* "your prompts", and editing a
// default is what archives it, surfaced via "View original" on the fork
// rather than a separate archive list - see supabase/README.md). Reachable
...
// renderAdminPage/renderCollectionsPage above. Lists the signed-in caller's
// archived *defaults* only (db.js's archiveDefault(); a personal prompt has
// no archived state, just Delete) - specifically excludes ones archived as
// a side effect of forking (prompt_overrides.fork_prompt_id set), since
// those are already reachable via the fork's own "View original" and
// unarchiving them here would just duplicate the still-existing fork back
// into the main list. Reachable at /archived/, linked from the sidebar
// below Sequences (hidden until signed in - auth.js's setNavAccountState).
...
<div class="container account-page archived-page">
  ...
  return renderLayout({
    ...
    description: 'Default prompts you’ve archived from your view.',
    active: 'archived',
    bodyHtml: body,
    extraScripts: script,
    data
