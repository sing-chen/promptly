  ...
  <div class="sidebar-section">
    <div class="sidebar-section-label-row">
      ...
      <span class="sidebar-section-label-group">
        ...
        ${fieldInfo('collections-info-hint', 'Collections group your prompts for a project or workflow, so you can pull them all up at once. Log in to create one.', {
  ...
  <div class="sidebar-section">
    <div class="sidebar-section-label-row">
      ...
      <span class="sidebar-section-label-group">
        ...
        ${fieldInfo('categories-info-hint', 'Categories sort your prompts. Browsing uses the shared set; logging in gives you your own — rename them, pick their colours, add ones that fit how you work.', {
...
function fieldInfo(id, hintText, { btnClass = '', hintClass = '', btnId = '', btnHidden = false } = {}) {
  return `<button type="button" class="field-info${btnClass ? ' ' + btnClass : ''}"${btnId ? ` id="${btnId}"` : ''}${btnHidden ? ' hidden' : ''} aria-describedby="${id}" aria-expanded="false">${ICON.info}<span class="sr-only">More info</span></button><span class="field-hint${hintClass ? ' ' + hintClass : ''}" id="${id}" role="tooltip">${esc(hintText)}</span>`;
}
