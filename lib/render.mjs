...
<div class="pd-seq-row">
  <span class="pd-seq-current">${esc(categoryLabel(prompt.sequence))} · Step ${prompt.sequence_step} of ${total}</span>
  <div class="pd-seq-nav">
    ${prev ? `<a href="/prompt/${prev.slug}/">${ICON.chevronLeft} Back</a>` : `<span style="color:var(--ink-faint);">${ICON.chevronLeft} Start</span>`}
    ${next ? `<a class="next" href="/prompt/${next.slug}/">Next ${ICON.chevronRight}</a>` : `<span style="color:var(--ink-faint);">End ${ICON.chevronRight}</span>`}
  </div>
</div>`;
  }
...
<div class="container pd-page">
  ...
  <header class="pd-header">
    <div class="pd-title-row">
      ...
      <span class="pd-cat-badge ${categoryHue(prompt.category)}">${esc(categoryLabel(prompt.category))}</span>
    </div>
