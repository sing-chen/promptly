...
// renderLayout) above whatever the page itself renders - holds the sidebar
// toggle (the only way to re-expand a collapsed sidebar, since the sidebar's
// own collapse button disappears along with it - public/scripts/
// sidebarCollapse.js) and the global controls that used to live in the
// sidebar (search, table/grid view toggle, theme). `pageTitle`/`pageSubtitle`
// are optional - pages with their own prominent heading (Prompt Detail,
// Account, Admin) omit them and just get the controls, rather than showing
// a duplicate title.
function renderMainHeader({ pageTitle, pageSubtitle } = {}) {
  return `
<header class="main-header">
  <div class="main-header-left">
    <button type="button" id="sidebar-toggle" class="icon-btn" aria-pressed="false" aria-label="Collapse sidebar" title="Collapse sidebar">
...
<div class="container">
  ...
  return renderLayout({ title: collection.title, description: collection.description, active: `collection:${collection.slug}`, bodyHtml: body, data, pageTitle: collection.title, pageSubtitle: collection.description });
}
...
// pattern BUILD_BRIEF_v4.md §6 lays out for /library/. No pageTitle here -
// auth.js's own renderSignedIn()/renderSignedOut() render their own "Account"
// heading into #account-root, so the generic header would just duplicate it.
