...
<section>
  ...
  const body = `<div class="container">${flows || '<p>No sequences yet.</p>'}</div>`;
  return renderLayout({ title: 'Sequences', description: 'Multi-step prompt chains.', active: 'sequences', bodyHtml: body, data, pageTitle: 'Sequences' });
}
...
<div class="container">
  ...
  return renderLayout({ title: name, description: `${name} sequence.`, active: 'sequences', bodyHtml: body, data, pageTitle: name, pageSubtitle: `${sequence.steps.length} step${sequence.steps.length === 1 ? '' : 's'}` });
}
