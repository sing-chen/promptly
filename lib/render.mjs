...
export function renderNav(active, data) {
  ...
  const link = (href, label, key, extra = '') =>
    `<a href="${href}"${activeKey === key ? ' aria-current="page"' : ''}>${label}${extra}</a>`;
  ...
  const categories = (data?.categories || []).filter(c => c.count > 0).map(c =>
    `<a href="/browse/${c.slug}/"${activeKey === 'category' && activeSlug === c.slug ? ' aria-current="page"' : ''}>${esc(categoryLabel(c.slug))}<span class="count">${c.count}</span></a>`
  ).join('');
...
<aside class="sidebar" id="sidebar" aria-label="Site navigation">
  ...
  <nav class="sidebar-primary-links">
    ${link('/', 'All Prompts', 'home')}
    ${link('/favorites/', 'Favorites', 'favorites', '<span id="nav-fav-count-num" class="sidebar-badge">0</span>')}
    ${link('/sequences/', 'Sequences', 'sequences')}
    <a href="/admin/" id="nav-admin-link"${activeKey === 'admin' ? ' aria-current="page"' : ''} hidden>Admin</a>
  </nav>
  ...
  <div class="np-modal" id="np-modal" role="dialog" aria-modal="true" aria-labelledby="np-title">
    ...
    <div class="np-page">
      <form id="np-form" class="account-form np-form">
        ...
        <label>
          <span class="np-label-row">Slug ${fieldInfo('np-slug-hint', 'The URL-friendly ID for this prompt (lowercase letters, numbers, hyphens). Auto-filled from the title - edit it directly if you want something different. Must be unique among your own prompts.')}</span>
          <input type="text" name="slug" required pattern="[a-z0-9]+(-[a-z0-9]+)*" placeholder="e.g. weekly-status-update">
        </label>
        <label>
