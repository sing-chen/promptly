...
export function renderPromptCard(p, ctx = {}) {
  ...
  return `
<a href="/prompt/${p.slug}/" class="prompt-card surface" data-categories="${esc(cats)}" data-title="${esc(p.title)}" data-updated="${esc(p.updated || p.added || '')}">
  <div class="card-top">
...
// `--accent` for the active fill. A single-option group acts as a plain
// toggle, not a radio - clicking it on and off is enough since there's
// nothing else in the group to exclude.
...
<div class="container">
  ...
  ${renderFilterToolbar([categoryGroup], { id: 'home-filters', sort: true, resultCount: true })}
  ${renderPromptTable(data.prompts, { sequenceTotals: data.sequenceTotals, gridId: 'home-grid', emptyText: 'No prompts yet.', data })}
  <p style="color:var(--ink-faint);font-size:12px;margin-top:20px;"><a href="/contributing/">Adding a prompt is just a file and a rebuild.</a></p>
</div>`;
...
// (chain-flow rail, not a browsed list). Each row carries the same data-*
// attributes cards use (title/updated) so the pill filter toolbar
// (filters.js) can read them directly, plus data-categories for the
// category pill filter. Full prompt data (body, notes, sequence info) is
// embedded as a JSON script tag for the quick-view modal (quickview.js) to
// read - a static site has no API to fetch it from on click.
export function renderPromptTableRows(prompts, ctx = {}) {
  return prompts.map(p => {
    ...
    const cats = p.categories.join(',');
    return `
<tr data-slug="${esc(p.slug)}" data-categories="${esc(cats)}" data-title="${esc(p.title)}" data-updated="${esc(p.updated || p.added || '')}" tabindex="0" role="button" aria-label="Open ${esc(p.title)}">
...
<div class="container">
  ...
  ${renderFilterToolbar([], { id: filterId, sort: true, resultCount: true })}
