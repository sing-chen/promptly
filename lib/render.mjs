...
// Category tile grid shared by Home and the Browse hub. Icon color picks up
// the category's hue from the same cat-clay/ochre/blue/plum/moss/rose
// palette used by cat-badge elsewhere (§9), instead of a flat --ink-faint,
// so the site's primary browse entry point matches the color language
// already established on cards/table/detail badges. `hideEmpty` drops
// zero-count categories - used on Home (the highest-traffic entry point,
// where a dead-end tile costs a wasted click) but not the Browse hub, whose
// whole purpose is being the complete category index.
function renderCategoryTiles(categories, { hideEmpty = false } = {}) {
  const cats = hideEmpty ? categories.filter(c => c.count > 0) : categories;
  return cats.map(c => `
<a href="/browse/${c.slug}/" class="surface" style="display:block;padding:16px;text-align:center;">
  <div style="color:var(--${categoryHue(c.slug)});display:flex;justify-content:center;margin-bottom:4px;">${ICON.category}</div>
...
export function renderHomePage(data) {
  const categoryTiles = renderCategoryTiles(data.categories, { hideEmpty: true });
...
export function renderBrowseHub(data) {
  const tiles = renderCategoryTiles(data.categories);
  const body = `
<div class="container">
