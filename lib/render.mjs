  ...
  <div class="sidebar-section">
    <div class="sidebar-section-label-row">
      <span class="sidebar-section-label-group">
        ...
        ${fieldInfo('collections-info-hint', 'A named group of prompts you put together for a project or workflow, so you can pull them all up at once.', { btnClass: 'sidebar-info-btn', hintClass: 'sidebar-info-hint' })}
      </span>
...
function fieldInfo(id, hintText, { btnClass = '', hintClass = '' } = {}) {
  return `<button type="button" class="field-info${btnClass ? ' ' + btnClass : ''}" aria-describedby="${id}">${ICON.info}<span class="sr-only">More info</span></button><span class="field-hint${hintClass ? ' ' + hintClass : ''}" id="${id}" role="tooltip">${esc(hintText)}</span>`;
}
...
// - every row/card gets an Edit button, but Delete only shows for a row the
// caller actually owns (editing a default forks it rather than editing it in
// place - see supabase/README.md "Admin curation..."). `ctx.currentUserId`
// is required alongside it to tell the two apart.
function personalizedActions(p, ctx) {
  ...
  return `
      <button type="button" class="icon-btn" data-edit-id="${esc(p.id)}" aria-label="Edit ${esc(p.title)}" title="Edit">${ICON.edit}</button>${del}`;
}
  ...
  <div class="why-compare">
    ...
    <div class="surface why-compare-col">
      ...
      <ul>
        ...
        <li>Write and edit your own prompts</li>
        <li>Fork a default prompt to customize it, or archive it if you'd rather not see it</li>
