  ...
  <div class="sidebar-section">
    <div class="sidebar-section-label-row">
      <!-- A heading, not a link (§9al). Both section labels lost their links
           ...
           The route to /collections/ did not disappear with the link - it
           moved into the hint below, which now carries a permanent one. That
           matters more than it sounds: the sidebar list links to
           /collections/ only once you HAVE a collection, so a signed-in user
           with none had no way there at all, which is exactly the state every
      ...
      <span class="sidebar-section-label-group">
        ...
        ${fieldInfo('collections-info-hint', 'Collections group your prompts for a project or workflow, so you can pull them all up at once.', {
          ...
          // Swapped by auth.js on session change. The signed-out text is the
          // callout's, which this replaces.
          cta: { id: 'collections-info-cta', href: '/why-sign-in/', text: 'Log in to create one →' }
        })}
      </span>
  ...
  <div class="sidebar-section">
    <div class="sidebar-section-label-row">
      <!-- Also a heading now (§9al), for the reason given on Collections
           ...
           everyone else.
           Signed out this leaves no route to /categories/ from the label, by
           design - the hint carries the "log in" path instead, and the gear
           takes over once there is an account to manage. -->
      <span class="sidebar-section-label-group">
        <span class="sidebar-section-label">Categories</span>
        ${fieldInfo('categories-info-hint', 'Categories sort your prompts. Browsing uses the shared set; your own account gets its own — rename them, pick their colours, add ones that fit how you work.', {
          btnClass: 'sidebar-info-btn', hintClass: 'sidebar-info-hint', btnId: 'categories-info-btn',
          cta: { id: 'categories-info-cta', href: '/why-sign-in/', text: 'Log in to make them yours →' }
        })}
      </span>
...
// `cta` adds a link inside the hint. It is a structured option rather than
// letting callers pass HTML because hintText stays esc()'d - the escaping is
// what makes this safe to hand arbitrary strings, and an `html: true` flag
// would quietly remove that for every caller who reached for it.
//
// A link in here only works because public/scripts/fieldInfo.js pins the hint
// open on click (§9al). While the hint was hover-only, anything inside it was
// unreachable: moving the pointer off the button to reach the link dismissed
// the thing you were reaching into. Do not add a cta to a hint on a page that
// does not load that script.
//
// The wrapper changes shape when there is a cta: <span> is fine for a run of
// text, but a block-level link inside an inline element is the sort of thing
// that renders differently across engines, so it becomes a <div>.
function fieldInfo(id, hintText, { btnClass = '', hintClass = '', btnId = '', btnHidden = false, cta = null } = {}) {
  const tag = cta ? 'div' : 'span';
  const ctaHtml = cta
    ? `<a class="field-hint-cta"${cta.id ? ` id="${cta.id}"` : ''} href="${esc(cta.href)}">${esc(cta.text)}</a>`
    : '';
  // aria-expanded reflects the pinned state; fieldInfo.js flips it. Present in
  // the static markup so the control never announces itself as a plain button
  // for the moment before the script runs.
  return `<button type="button" class="field-info${btnClass ? ' ' + btnClass : ''}"${btnId ? ` id="${btnId}"` : ''}${btnHidden ? ' hidden' : ''} aria-describedby="${id}" aria-expanded="false">${ICON.info}<span class="sr-only">More info</span></button><${tag} class="field-hint${hintClass ? ' ' + hintClass : ''}" id="${id}" role="tooltip">${esc(hintText)}${ctaHtml}</${tag}>`;
}
