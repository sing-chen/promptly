...
function personalizedActions(p, ctx) {
  ...
  const archive = p.is_curated ? `<button type="button" class="icon-btn" data-archive-id="${esc(p.id)}" aria-label="Archive ${esc(p.title)}" title="Archive">${ICON.archive}</button>` : '';
  const del = isOwn ? `<button type="button" class="icon-btn" data-delete-id="${esc(p.id)}" aria-label="Delete ${esc(p.title)}" title="Delete">${ICON.trash}</button>` : '';
  return `
...
<a href="/prompt/${p.slug}/" class="prompt-card surface" data-slug="${esc(p.slug)}" data-categories="${esc(cats)}">
  <div class="card-top">
    ...
    <span class="card-actions">
      ...
      <button type="button" class="icon-btn" data-copy-target="prompt-copy-${esc(p.slug)}" aria-label="Copy ${esc(p.title)}">${ICON.copy}</button>${personalizedActions(p, ctx)}
    </span>
  </div>
