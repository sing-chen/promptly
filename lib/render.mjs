...
<div class="container prose-page legal-page">
  ...
  <ul>
    ...
    before you have signed in or done anything, and it happens even if you never
  ...
  without asking: your library is yours to edit and delete at any time. For
  anything else, email <a href="mailto:${esc(d.email)}">${esc(d.email)}</a> and
  we will respond within one month.</p>
...
<div class="container prose-page why-sign-in-page">
  ...
  require an account. Signing in gives you a copy of the whole catalog that is
  genuinely yours to change.</p>
  ...
  way. When you sign up, every published prompt in the catalog is
  ...
  <p>A first name, an email address and a password. The first name is only used
  to greet you on your account page. There is no payment method to enter —
  ...
  <!-- Hidden once you are signed in, by auth.js's setNavAccountState - the
       same place #new-prompt-btn and #nav-signout-btn are toggled, rather
       than a second session check on this one page.

       Unlike those two it starts VISIBLE and is hidden on session, because
       the inverse is true here: this page exists to persuade signed-out
       visitors, so they are the ones who must never see a flash of the wrong
       state. A signed-in visitor may briefly see the button before the
       toggle runs, which is the right way round for a page they have little
       reason to be on. -->
  <p id="why-signin-cta"><a href="/account/" class="btn btn-primary">Sign in / sign up</a></p>
</div>`;
  return renderLayout({
    title: 'Why sign in?',
