...
// The per-user ceiling (OPEN_ITEMS.md B3). A sanity limit, not a quota:
// nothing breaks at 21, but the sidebar list and the filter pill row are laid
// out for roughly a dozen entries and nobody has designed what either looks
// like at 200.
//
// This number is duplicated in the database, as a clause on the
// categories_insert RLS policy (0007_category_limit.sql). There is no way to
// share one constant across Postgres and the browser, so raising the limit
// means editing both. They disagree safely in one direction only: raise this
// without the migration and /categories/ will offer a create that the database
// then refuses with a raw RLS error.
//
// Two things this limit deliberately does NOT do, both explained at length in
// 0007: it does not apply to ensure_seeded() (seeding must never fail, so a
// user can legitimately end up above 20), and it does not exempt the admin
// (whose library *is* the catalog, so capping them is what keeps a freshly
// seeded account under the ceiling).
