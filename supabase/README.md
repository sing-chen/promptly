...
- **Known gap, confirmed by the admin sign-in test above, not a bug**: neither a regular prompt nor a published default shows up under "All Prompts," any category page, or anywhere else on the site after creation. Expected — those are static pages `scripts/build.mjs` pre-renders from `prompts/*.md` at build time, and nothing reads Supabase yet for either the anonymous site (needs the build-time Supabase read, next item below) or a signed-in user's own list (needs `/library/`, which doesn't exist). A newly created prompt is real and persisted (confirmed via `/admin/` surviving a refresh) — there's just no view that queries for it outside `/admin/`'s own curated-only list.
