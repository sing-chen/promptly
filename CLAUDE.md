...
**[OPEN_ITEMS.md](OPEN_ITEMS.md) is the single register of everything outstanding** — not built, not decided, or deliberately deferred, ordered by what has to be true before each item matters. It replaced the five separate lists the briefs used to carry (v4 §7, v5 §9, v6 §9, BUILD_BRIEF.md §12, supabase/README.md's Next steps), which had drifted apart and in two places contradicted each other. Check it before planning work; don't re-derive a to-do list from the briefs.
...
[BUILD_BRIEF_v4.md](BUILD_BRIEF_v4.md) is **historical** — the account-tier plan as originally conceived (Supabase auth, admin-curated defaults, per-user forks). Its §4/§5/§11 describe a model that no longer exists. Read it for the original *why* (§1, §2, §10), never for how anything works now. Its §7 was the original open-items register and is now a pointer to OPEN_ITEMS.md — two of its entries had gone stale where they sat.
...
The UI halves are logged as BUILD_BRIEF.md §9r (v5) and §9s (v6), following that document's pass-by-pass convention, with §9t covering the mobile-layout repairs that followed §9s.

*(A paragraph here used to say v6 was "a design pass, not built", and that categories were still the hardcoded `CATEGORIES` array in `lib/schema.mjs`. It was written before the pass shipped and never updated, so this file contradicted itself for two paragraphs — see the entry above, which is the accurate one. `lib/schema.mjs` now opens with "This file used to hold CATEGORIES". Removed rather than corrected, because it duplicated what the v6 entry already says.)*
