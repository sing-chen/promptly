...
**Not verified, and cannot be from here**: that `delete from auth.users` actually succeeds. `auth` is Supabase's own schema and the supported path is the admin API; this is convention, not contract. It must be proven on a throwaway non-admin signup before it is trusted, and re-checked after any Supabase auth upgrade.
