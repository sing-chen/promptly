...
The **unpublish round trip is proven**, and by an inference worth keeping rather than by a claim: two screenshots of the *same* prompt show the row button reading `Unpublish` in one and `Publish` in the other, which is only reachable if the confirm ran through `unpublishPrompt()` and the list reloaded against the new state. The **publish** direction is *not* established by that evidence — the row could have been published before the session began — and it is deliberately still recorded as open rather than rounded up, since "both dialogs work" and "both actions complete" are different claims and this project has been bitten three times by the gap between them.
