...
Breadcrumb (Home > [Category] — was Home / Browse / [Category] before §9g removed the Browse hub and §9i switched the separator to `>`) → heading + one-line category description (`CATEGORY_DESCRIPTIONS`, `lib/schema.mjs`, §9i) → result table (§9b) with quick-view modal, no filter toolbar at all. (Model/Complexity pills, then Chain + Sort, sat in a toolbar here at various points; all removed — Chain and Sort in §9i, the rest earlier in §9c.)
...
- **Prompt authoring assistance.** *(Premise partly overtaken by events.)* This was written when prompts were hand-authored Markdown in `/prompts`, and proposed a Claude Code skill to quality-check frontmatter before a file landed there. That workflow is gone: catalog prompts are now authored in the app and stored in Supabase (`supabase/README.md`, "There is no markdown catalog"), and `/prompts` is retired test content the build no longer reads. What survives is the underlying want — **a quality check before a prompt reaches the catalog** (required fields, ≥1 category, house style) — which would now belong in the New Prompt modal or the `/admin/` publish step rather than in a file-authoring tool. §1's "no live submission form or moderation queue" still holds for *other* people's prompts; see BUILD_BRIEF_v5.md §9 for the separate, logged idea of a user submission/moderation flow. The `/contributing` page documenting the old raw-file workflow was removed in §9j.
