...
Account                           /account   (sign in/up, library stats §9r)
                                     (#sign-up / #forgot open those modes; ?confirmed=1 is where an activation link lands, §9ar)
Set a new password                /reset-password  (§9ar — unlinked by design; where the reset email lands)
...
## 9an. /collections/ Rebuilt on /categories/ (new)

Three things, one theme: two pages that manage the same kind of thing had arrived at different answers to every question, for no reason anyone could state.

**`/collections/` now mirrors `/categories/` structure for structure.** Header with a title and a subtitle explaining the feature (it had *neither* — the page opened straight onto a form), a "+ New collection" button opening an inline editor, then a draggable list of rows with Edit and Delete. It was previously a permanently-open create form above a grid of cards.

**The reported styling bug was a `<textarea>` in a form of `<input>`s.** Description is now `<input type="text">`, exactly as the category editor's is — same control for the same job, a one-liner shown beside the row. Verified: every computed property of the two fields now matches, tag included.

**Reordering needed a migration, and it is not applied.** `collections` had no `position` column; `categories` has had one since `0006`. [`0009_collection_position.sql`](supabase/migrations/0009_collection_position.sql) adds it in the same shape, backfills by title so the first render after applying does not visibly shuffle anything, and indexes `(user_id, position)`. **This must be run before the deploy is usable** — `loadCollections()` now orders by `position`, so without the column the page fails outright rather than merely losing the reorder. Recorded in `supabase/README.md` under a "not yet applied" heading.

**The prompts-in-a-collection problem.** Each member renders as a real `<a href="/prompt/…/">` carrying `data-slug`, and the whole list is handed to `initQuickView('col-list')`. Both halves earn their place: the `href` means middle-click, open-in-new-tab and no-JS all work and the element is honestly a link; the `data-slug` is what the quick-view controller binds to, and its handler calls `preventDefault()` so an ordinary click opens the modal in place. That is the same trick the sequence rail already uses, not a new idea.

One delegated handler covers every prompt in every collection, because `initQuickView` binds to the element it is given. Re-calling it after each repaint is correct rather than leaky: `render()` replaces `#col-list` wholesale, so the previous node and its listeners are gone. `renderCollectionsPage` had to start emitting `renderQuickViewModal()` — without that markup the call finds no `#qv-backdrop` and silently does nothing.

**The `(i)` hints drop their "log in to…" half once there is a session.** Advice aimed at someone who has already taken it is noise in a tooltip trying to be brief. Split into a `guest` span that `auth.js` hides, rather than swapping strings from JS, so the text stays in one place — the markup. Applied to Categories as well as Collections: the same sentence was doing the same wrong thing there.

**CSS is grouped, not duplicated.** `.col-*` selectors join the existing `.cat-*` rules (`.cat-list, .col-list`, `.cat-row, .col-row-head`, and so on) rather than getting a parallel copy. Two lists meant to look identical should fail identically when one of these numbers changes; a parallel rule set is precisely how they drift apart. The one real shape difference is kept explicit: a collection row is a flex column (head, members, add control) where a category row is just the grid, and a collection has no colour of its own, so its title takes `.card-title`'s treatment instead of a badge. The dead `.lib-collection-*` block went with the old markup.

**Verified**: both page headers now render the same `main-header-titles` structure; Title and Description compute identically; clicking a member link opens the quick-view modal with the right prompt and does **not** navigate away; row layout resolves (grid head, column row, wrapping members) and collapses to two columns at 375px with no horizontal overflow; the guest spans carry the right text and are visible signed out.

**Not verified**: everything behind a session — the list itself, the editor, drag-to-reorder, add/remove member — was checked against representative markup injected into the real page, not against real data, because this environment cannot create a session. The reorder in particular is unexercised against a database that does not yet have the column.

---

## 9ao. The Sidebar Honours the Collection Order (new)

§9an added drag-to-reorder to `/collections/`. The sidebar ignored it, and the cause was one line: `collectionsNav.js` re-sorted by `title.localeCompare()` before rendering, discarding the order `loadCollections()` had just been changed to return. **The arrangement saved correctly and the sidebar threw it away**, which is the worst shape for this kind of bug — nothing errors, the feature simply appears not to work.

The fix is a deletion. `loadCollections()` orders by `position` with title as the tie-break, exactly as `loadMyCategories()` does, so the right thing for the renderer to do is what `categoriesNav.js` has always done: nothing. Worth noting the sort was not wrong when it was written — there was no `position` column then, and alphabetical was the only order available. It became wrong when `0009` gave the data an order of its own, and nothing pointed at it.

**Collections now carry a folder icon.** Chosen over the alternatives on a specific ground rather than by default: §9an's rewording made the tooltip read "Collections group your prompts into folders", so the icon restates the copy instead of introducing a second metaphor. A paperclip and an overlapping-cards mark were the other candidates — the first reads as "attachment" everywhere else in software, the second sits too close to All Prompts' existing layers icon at 16px.

`ICON.category` supplied the shape. It was a folder defined for a sidebar list that uses colour dots and referenced by nothing — dead since §9s. Renamed to `collection` rather than duplicated, so the map does not now carry two folders.

**Colour per collection was considered and declined.** Categories' dots differ per row and therefore carry information; a folder repeated down the list does not. The honest options were per-collection colour (a second migration, a swatch picker, a longer editor) or accepting a decorative marker. Decided: not now — collections are few and distinctly named, and colouring them would make the two sidebar lists harder to tell apart rather than easier. Recorded so the repetition is understood as a decision rather than an oversight.

**Verified**: given rows in position order, the renderer preserves them where the old alphabetical sort would have reordered them; the icon renders 16×16 in the same slot as the category dots, with labels landing on the identical x-position in both lists; contrast 3.03:1 light / 3.73:1 dark, above the 3:1 non-text floor.

Noted, not changed: that 3.03:1 is `.nav-icon`'s inherited `--ink-faint`, shared with All Prompts, Favorites, Sequences and Archived. It passes for a non-text mark and the labels beside it are `--ink-soft` at 5.89:1, so this is not the D5b failure §9ai and §9aj fixed — but it is the same token, and any future retune of it should sweep here too.

---

## 9ap. A Collection Link Shows the Collection (new)

Clicking a collection in the sidebar opened `/collections/` — the management page. It answered "show me what is in this" with a form for renaming it. Categories have never worked that way: `categoriesNav.js` sends every link to `/?cat=<slug>`, Home filtered. Collections now do the same, at `/?collection=<slug>`.

**Collections became a second pill group** rather than a bespoke filtered view. The toolbar was already built for this — `initFilterToolbar` tracks state per `data-group` and documents OR-within-group, AND-across-groups — but `initTableFilterToolbar`'s `apply()` only ever read the `category` group. It now reads both, so "Marketing prompts that are in my Launch kit" is expressible, and the semantics are the ones the toolbar always claimed. Verified against real rows: 10 → 3 on a collection, still 3 when a second collection is added (OR), 1 when a category is added on top (AND).

**Rows carry `data-collections`, but only when signed in.** `collectionAttr()` emits nothing unless the caller supplies the map, because the static build has no collections at all — they are user-generated, per-account, never curated. Confirmed: the anonymous pages ship zero `data-collections` attributes while keeping all ten `data-categories`.

**Collections joined the personalization cache** rather than being fetched by whoever needs them. They are now read on every personalized page, and `personalizeData.js` is already the one place per-session reads are cached and invalidated. Unlike categories they need not wait for seeding — `ensure_seeded()` creates no collections, so there is no empty-list race to lose. The cache also builds the inverse index the rows need: `loadCollections()` returns collections holding prompt ids, while each row needs the collections *it* belongs to.

**`ctx.collectionSlugsByPromptId` is refreshed on every paint**, not captured once. Adding a prompt to a collection fires `personalization:changed`, which re-reads and repaints; a ctx still holding the page-load map would re-render rows with stale membership and the filter would quietly disagree with `/collections/`.

**The URL lookup is now scoped by group.** `applyCatFromUrl` became `applyFiltersFromUrl` and matches on `[data-group][data-value]` rather than value alone. A category and a collection are free to share a slug — nothing prevents it, they are separate tables with separate uniqueness — and "launch" meaning both would otherwise activate whichever pill came first in the DOM. Verified with a deliberately colliding pair: scoped picks the collection, unscoped picks the category.

**Collection pills carry no colour**, which is not an omission. `renderFilterToolbar` only adds `.is-hued` when a colour is supplied, so these render plain and read as a different axis at a glance — which they are. It also follows §9ao's decision not to give collections colour.

A user with no collections sees exactly the toolbar they saw before: empty-count collections are dropped by the same filter categories use, and `renderFilterToolbar` omits a group with no surviving options.

**Also verified this pass**: `0009` is applied. Confirmed against the live project rather than taken on trust — `select=id,title,position&order=position.asc` returns 200 where a fabricated column returns `42703`, so `loadCollections()`'s ordering has the column it needs.

**Not verified**: the signed-in page end to end. The filter logic, the URL scoping and the group rendering were each exercised against the real toolbar and real rows, but with membership stamped on by hand — this environment cannot create a session.

---

## 9aq. Three Screens, Not One Wearing Three Hats (new)

§9ap put collection filtering on Home and that was the wrong home for it. Home was already serving as three screens: All Prompts, Prompts by Category (`/?cat=`) and Prompts by Collection (`/?collection=`). One page carried both pill rows, and called itself **All Prompts** while showing a filtered subset — which is why the toolbar read as complicated. It was not the pills; it was that one screen was answering three questions.

**Split into `/`, `/by-category/` and `/by-collection/`.** Each carries exactly one filter row: Home keeps categories (its collection row is gone), `/by-category/` categories, `/by-collection/` collections. Each states what it is in its own header.

**The "N new prompts" callout is Home's alone.** It belongs to the screen that means *everything*; on a filtered view it announces prompts that may not be in the filtered set at all. The new pages simply never render it, so nothing had to be suppressed.

**`filterGroups` is a page option, defaulting to `['category']`.** That default is what leaves Favorites and the Category pages exactly as they were — they predate this and ask for nothing.

**One function builds both new pages.** They differ in four strings and one group key; two near-identical copies is how the next change lands in only one of them.

**`applyFiltersFromUrl` moved into `filters.js` and is now shared.** It was private to `personalize.js`, which runs only for signed-in callers — so an anonymous visitor reaching `/by-category/?cat=writing` got the whole catalog and a query parameter the page silently ignored. Both callers now use the one implementation: the page's inline script, before the first `apply()` so the page paints already-filtered rather than visibly narrowing, and `personalize.js` again after each toolbar rebuild, once the caller's own pills exist. Verified signed out: `?cat=writing` pre-activates the pill and cuts 10 rows to 2.

**"In collections" on the prompt modal and the full page.** Below the hairline and above Created/Modified, as asked. Hidden unless the prompt is in at least one collection — a "none" row would be noise on the many prompts in none, and meaningless to anonymous visitors who have none at all. The chips link to `/by-collection/?collection=<slug>`, so the detail view leads back into the filtered screen.

Two call sites, one helper (`collectionsForPrompt`). The modal already holds a personalization object and re-renders per prompt; the static page has one prompt and must go and fetch, which is what `promptCollections.js` does. **The static page matches by slug, not id** — it is the *catalog's* copy, and under owned copies the reader holds a different row with a different id for the same prompt. Slug is what survives the copy, the same reason the export uses it.

**Verified**: all three pages render distinct titles, subtitles and pill rows, with the callout only on Home; `/by-collection/` renders no pill group at all for anonymous visitors and does not error; the collections row sits between the divider and the dates in the generated prompt page and ships hidden; chips read 12.14:1.

**Not verified**: anything needing a session — the collections row actually populating, and the per-page pills rebuilt from the caller's own data.

A note on the environment rather than the code: three modules appeared to fail importing mid-session, which sent me looking for a dependency fault that did not exist. The shared dev server was dropping connections under rapid parallel imports. Same lesson as §9ai's frozen transitions — **an impossible result is a broken measurement until proven otherwise**, and this is now the third time in this run.

---

## 9ar. The Email Half of the Account Tier (new)

[OPEN_ITEMS.md](OPEN_ITEMS.md) A1 was filed as "configure SMTP", and A2 as "the screens have never been designed". They are the same job seen from two ends, and this pass builds the end that lives in this repo: everything that happens either side of an email. The dashboard end — custom SMTP, "Confirm email", the templates, the redirect allow-list — is written up as a runbook at [`supabase/email-templates/README.md`](supabase/email-templates/README.md) rather than described here, because it is configuration and configuration belongs where it can be followed step by step.

**Confirmation is required before log-in, and the app is written for that rather than around it.** With "Confirm email" on, a successful `signUp()` returns *no session* — that absence is the signal an email went out. The code checks for it explicitly and, if a session ever does come back, says "Account created. You are logged in." instead of sending the user off to wait for mail nobody sent. It is three lines guarding against a dashboard toggle changing underneath the client, which is the class of failure this project keeps meeting (the deploy hook, the leaked-password setting, the password minimum).

**The welcome email is the confirmation email.** A separate welcome message sounds like a small addition and is not: Supabase's templates only cover mail Supabase Auth itself sends, so a standalone one needs its own sender and its own trigger. It would also arrive seconds after the email that must be acted on, which is how the one that matters gets ignored — and a welcome-only mail that is not required for the account to work is marketing by another name, which [OPEN_ITEMS.md](OPEN_ITEMS.md) E6 blocks for reasons unrelated to plumbing. So the confirmation template carries the welcome: what Promptly is, what the library will contain, and one button.

**"Check your email" is a panel that replaces the form, not a message under it.** A status line under a still-filled-in form invites a second submit, which is how someone ends up with four links in their inbox and uses the oldest. The panel names the address it went to, gives the spam-folder advice first because that is the one that actually works, and offers a resend on a 60-second countdown — matched to Supabase's own per-address throttle, so the button never invites a press that is guaranteed to fail. One renderer serves both the sign-up and the reset case; they differ in a heading and a sentence.

**The failure worth intercepting is `email_not_confirmed`.** Logging in against an unconfirmed address is not something the user can fix by trying again, and the action it calls for — send that link again — is not on the screen. That error alone gets the panel treatment instead of its raw message.

**Forgotten password is a third mode of the same form, not a third tab.** It is a detour off log-in rather than a peer of log-in and sign-up, so it takes the tab row away and states its own heading. Every hidden/required pair moves together, for the reason §9v recorded about the first-name field: a hidden `required` input is an invalid control the browser cannot focus, so the form refuses to submit with nothing on screen to explain why. That now applies to the password field too, which forgot-mode hides.

**`/reset-password/` is a new route, and the only one in the site that is deliberately unlinked.** Every path to it is an emailed link or a signed-in visitor typing it. Two things about it are worth not re-deriving: the recovery link produces a *real session* before the page runs (supabase-js consumes the fragment during initialisation), so setting the password is an ordinary `updateUser()` and there is no token to handle; and because of that, a caller who is already logged in gets the same working form — "change my password" and "I forgot my password" end in the same place rather than in two copies of one screen.

**The dead-link fragment is read synchronously, at module load.** An expired or already-used link comes back as `#error=…&error_code=otp_expired`, not as a rejected promise. supabase-js strips that fragment as part of its own async initialisation, so anything that waits — including `DOMContentLoaded` — finds nothing, and an expired link becomes indistinguishable from a working one that quietly did nothing. `LINK_ERROR` is therefore computed at the top of `auth.js`, before the first `await` on the page, and exported for `/reset-password/` to reuse.

**Password rules moved to `public/scripts/password.js`.** `/reset-password/` chooses a *new* password, so every rule that applied at sign-up applies again. The alternative was a second copy, which fails in the quiet direction: the copy drifts and the form starts disagreeing with what Supabase will accept. §9ah's decisions are unchanged, just relocated — length-first, no composition rules, the meter advisory.

**Verified in-browser**, against the live Supabase project: all three form modes toggle the right fields, labels and `required` flags; a reset request for a non-existent address renders the panel with the countdown running (Supabase deliberately returns success either way, so the form cannot be used to discover which addresses have accounts); `/reset-password/` with no session shows the expired-link state, and with a fabricated `otp_expired` fragment shows Supabase's reason instead; `/account/?confirmed=1` shows the confirmation message and strips the parameter from the URL so a refresh does not re-announce it; `#forgot` and `#sign-up` deep links both open the right mode. Contrast measured on fresh nodes in both themes: `.linkish` 5.76 light / 7.99 dark on `--paper` and 5.19 / 8.20 on the panel's `--well`; the checklist and the sent-to line 4.88 / 8.52.

**Not verified, and not verifiable from here**: that any of it delivers an email. That is the runbook's seven-step sequence and it needs the dashboard first.

The run's recurring lesson turned up twice more, both times in the *measurement* rather than the code. The browser tool's navigation drops a fragment on a same-document move, so `#forgot` and the error-hash both read as "not implemented" until they were loaded properly; and a computed colour read on a node inherited from a previous paint came back as black on transparent, a ratio of 1.00, which is not a colour scheme any stylesheet in this project could produce. Fourth and fifth instances of §9ai's rule.

---

## 9as. A Session Is Not Evidence That the Link Worked (new)

§9ar shipped, SMTP went in behind it (Gmail as a deliberate interim), and the seven-step sequence in the runbook was run against the live project. Six steps passed, mail landed in the **inbox on both Gmail and Outlook** — the one fact no amount of local testing could produce — and step seven, "follow the spent reset link again and see it refused", showed the password form instead.

**The test instruction was wrong, and the code had a real defect behind it.** Both are worth separating, because only one of them was a bug.

The instruction: step 6 leaves you *signed in* by design, and `/reset-password/` serves any signed-in caller the form regardless of what brought them there — that is §9ar's deliberate merge of "change my password" with "I forgot my password". So a second click while signed in was always going to show a form. The step only tests single-use if you log out first, and now says so.

The defect: the page decided everything from `getSession()`, so when a session existed it **never looked at the link at all** — and silently discarded `LINK_ERROR`. A visitor who clicked a dead link got a working form and no indication anything was wrong, which is the precise failure `LINK_ERROR` was written to prevent, reintroduced one branch away from where it was fixed. **A session is not evidence that the link worked**: someone already logged in has one before the link is considered.

**Fixed by making the two questions separate.** `LINK_ARRIVAL` joins `LINK_ERROR` on the same synchronous fragment read: one says *the link was dead*, the other says *this visit came from a link at all*, which is otherwise unrecoverable once supabase-js strips the fragment. With a session and a dead link, the page now shows the form **and** a banner saying the link was spent and that no new one is needed. Keeping the form is the deliberate half: they are signed in, so they can genuinely change their password, and withdrawing that to punish a stale link helps nobody.

**A second thing fell out of it.** The `LINK_ERROR` strings used to end "request a new one below", which is right on `/account/` and exactly backwards on the already-signed-in screen — where the correct advice is *carry on, you need nothing*. The consuming screen had started stripping the suffix back off with a regex, which is the shape of thing that later fails silently. The strings are now **diagnoses only** and each screen appends its own instruction, because what to do next is a property of the screen, not of the error.

**`LINK_ARRIVAL` also gets the heading right**: "Choose a new password" for someone mid-reset, "Change your password" for someone who typed the URL while logged in. Same form, same rules; calling the second one a reset implies a process they never started.

**Verified in-browser** across all three states, the middle one by fabricating a session in `localStorage` rather than by holding a real one, since the build tooling cannot log in: signed out with a dead fragment gives the refusal screen; signed in with a dead fragment gives form-plus-banner and the "Choose" heading; signed in with no fragment gives the form and the "Change" heading, no banner.

The lesson generalises past this page, and is the one to carry: **when a check has two possible sources of truth, decide which question it is actually asking.** "Can this person set a password?" and "did this link work?" have different answers, and collapsing them into one `if (session)` is how a correct-looking screen tells a confident lie.

---

## 9at. Three Measures, Not One (new)

The "check your email" panel inherited `.account-page`'s 380px, which is the right width for a stack of input fields and the wrong one for three sentences and a bulleted list. At ~40 characters a line the spam-folder advice wrapped into a column and read as squashed — which is exactly what it was.

**A third measure, `.is-message` at 560px** (~62 characters, inside the 45–75 that reads comfortably), joining the default 380 and `.is-dashboard`'s 940. Applied by JS to the screens that are prose — the sent-email panel, the dead-link screen, the password-changed confirmation, and the reset form *when it carries the dead-link banner* — rather than by widening `.account-page` outright, because the form's narrow measure is correct and is part of what makes it read as a form. The width follows what is on the page rather than which function drew it, and is removed again on the way back.

## 9au. A Findable Reveal Instead of a Confirm Field; a Door to a Room That Existed (new)

**Confirm-password was asked for and declined**, and the reasoning is the point rather than the conclusion. The case for it is real: people do not always click the eye, so a typo goes unnoticed. Three things outweighed it.

The first is specific to this project and dates from three days ago: **a mistyped password used to be an unrecoverable account, and as of §9ar it is not.** That was the strongest argument for a second field, and shipping password reset removed it. A typo now costs one "Forgotten your password?" and one email.

The second is the measured evidence, which runs the other way. GOV.UK's Design System dropped confirm-password in favour of a reveal; NIST SP 800-63B recommends allowing paste and offering reveal rather than confirmation. The observed failure is that people paste into one field and type into the other, so the control generates mismatch errors at roughly the rate it prevents typos — and it is actively hostile to password managers, which fill one field and leave the other empty.

The third is consistency with what is already locked here. §9ai put the reveal on both tabs as *the* answer to "did what I typed land correctly", and §9ah's whole stance is evidence-led rather than habitual. A confirm field is the same class of decision as "must contain a symbol": it feels careful, and it is the one everybody copies without checking.

**What the objection was actually about was prominence, so that is what changed.** The reveal now carries the visible word *Show* / *Hide* on the two screens where a password is being **chosen** — sign-up and `/reset-password/` — and stays a bare icon on the log-in tab, where you are recalling a password rather than composing one. The word is in the markup from first paint and hidden by CSS, so nothing appears a frame late; the accessible name stays the full phrase, since "Show" alone names nothing read aloud. The button sizes to a `min-width` covering the wider of the two words: measured at 70px and 66px, and a control whose left edge moves 4px under the cursor that just clicked it feels unreliable for no reason anyone can name.

**Change password on `/account/` is a door, not a room.** The capability already existed — §9ar made `/reset-password/` serve a signed-in caller deliberately, so "I forgot it" and "I want to change it" end in one screen rather than two copies of one — but nothing linked to it, which made a routine action look like it required losing access first. A **link**, not a form and not a button, for the same reason the sidebar's Export entry is one (§9af): it navigates, and a second password form on `/account/` would be precisely the duplicate the shared page exists to avoid. `LINK_ARRIVAL` (§9as) already had the heading right for this route — "Change your password" rather than "Choose a new password", because the latter implies a process they never started.

**Verified in-browser**: the label shows on sign-up and hides on log-in, with the input's right padding moving 40 → 84px in step; the toggle holds 72px across both states with its left edge fixed; the label measures 5.42:1 at rest and 5.76:1 pressed; `/reset-password/` carries the label unconditionally and titles itself "Change your password" for a session that did not arrive by link; and `/account/` renders exactly one Password section, between Export and Delete.
...
**Unpublish gets one too, and that is not new weight.** `newPrompt.js` has confirmed from the Edit modal's curation checkbox all along; `/admin/` was the surface that didn't.

**But the two are not the same operation, and reusing the wording asserted that they were.** Caught while writing the steps for the parity test, which is the part worth keeping: `unpublishPrompt()` sets `published: false` and stops, so the row stays `is_curated`, reverts to a **catalog draft**, and is still listed on `/admin/`. `demoteFromCatalog()` also clears `is_curated` — *that* one leaves the catalog, becomes a personal prompt, and disappears from the page. Copying the modal's **"Remove from the catalog?"** onto `/admin/` therefore told the admin the prompt was leaving something it stays in. The title is now **"Unpublish this prompt?"** and the body says it returns to draft, pointing at the checkbox for the other operation. Note the body was already accurate; only the heading overstated — the harder kind to catch, because the paragraph beneath it reads correctly and the heading is what gets skimmed. The generalisation: **two controls sharing a confirmation is evidence they might be the same action, not a reason to assume it.**
...
**Confirmed in production**, on the deploy of `559c7ea`, which was the half no script here could reach — `/admin/` is admin-gated, so it needed a real log-in. Both dialogs render with the right wording, and the colour split survived the build: **Unpublish red, Publish to everyone green**. That last detail was the one worth checking, because `danger: false` is a flag passed through a shared helper and a regression in it would be invisible on the publish path until someone noticed the wrong dialog looked destructive.

**All seven checks pass**, run against production over the course of the session: the publish dialog and its cancel/Escape/backdrop paths, the publish round trip through to `publishPrompt()`, a title carrying `"` and `&` arriving in the dialog as real characters rather than entities, and finally the Edit-modal demote — which was the one that took three attempts to test properly.

**That last one is the part worth recording, because the test was wrong twice before the feature was.** The demote confirm is gated on `editingPrompt?.published`, and two runs produced no dialog and an identical success panel, which read as a failure. Both were **correct behaviour against a draft**: the success message *"removed from the catalog"* fires on the `wasCurated && !makeCurated` branch, which turns on `is_curated`, **not** `published` — so a draft demoting shows exactly the same panel as a published prompt demoting, with no dialog, rightly. The screen could not distinguish the two states, and neither could the tester. What settled it was instrumenting the event rather than inspecting the outcome — a one-line `prompt:edit-request` listener printing `published`/`is_curated` at the moment the modal opens — which returned `published: true`, after which the dialog appeared exactly as specified. **A success message shared between two paths is not evidence of which path ran**, and when a test's precondition is invisible on screen, instrument the input rather than re-reading the output.

*One thing to hold about the session it was tested in:* a `401` appeared on `loadFavorites()` partway through — PostgREST rejects a JWT with 401, whereas an RLS denial returns 200 and an empty array, so that was token expiry rather than permissions. Results taken through a stale session are not results; the run above was re-done on a fresh one.
...
**Verified** structurally before deploy: header and body cell counts match at 5 (a column added to one and not the other is the obvious way to break this), the button renders at 28×28 matching the row-action family, its icon draws, it does not collide with the publish button, and the horizontal scroll at narrow widths is pre-existing — measured with the new column removed, and `.table-wrap` still overflowed.
