...
**B4. Make publish feel weightier in `/admin/`.** **Done — moved to H.** Both publish and
unpublish now confirm, through the existing `confirmDialog()`. The **N** this entry asked
for was deliberately not built and should not be re-attempted from client code — see H for
why the only number a browser session can honestly produce is zero.
...
| **B4 — publish feels as light as any other toggle** | **Built** in §9av. Publish and unpublish both confirm, via the `confirmDialog()` shell `renderLayout` already puts on every page. Publish is `danger: false` (a `btn-primary` confirm) — consequential, not destructive. Unpublish confirming as well is not extra weight: `newPrompt.js` has confirmed that same action from the Edit modal all along, so `/admin/` was the surface disagreeing with itself. **The "N libraries" count in the original entry was deliberately not built, and this is the part not to re-derive:** `catalog_grants` is RLS'd to `user_id = auth.uid()` (`0004`) and an admin holds no grants at all, since `ensure_seeded()` no-ops for them — so the only count reachable from a browser session is **zero**, which reads as reassurance in the one dialog meant to give pause. A true N needs a SECURITY DEFINER RPC and a migration; it is a reasonable follow-on, but it is not a client-side fix and attempting one there will produce a confidently wrong number. The wording names the consequence instead. **Verified end to end in production**, all seven checks: both dialogs and their colour split (Unpublish red, Publish to everyone green), cancel/Escape/backdrop leaving the row clickable, the publish round trip, a title with `"` and `&` arriving unescaped, and the Edit-modal demote. **The demote test was wrong twice before it was right**, and that is the reusable part: it produced no dialog and an identical success panel on two runs, read as a failure, and was correct behaviour against a **draft** — the message *"removed from the catalog"* turns on `is_curated`, not `published`, so a draft and a published prompt demote to the same screen. The precondition was invisible in the UI. Instrumenting the input settled it (a one-line `prompt:edit-request` listener logging `published`), where re-reading the output could not have. **A success message shared by two paths is not evidence of which path ran.** |
